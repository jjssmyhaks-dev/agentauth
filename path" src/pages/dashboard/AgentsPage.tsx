...
export default function AgentsPage() {
  ...
  const handleCreate = () => {
    ...
    const fp = "SHA256:" + Math.random().toString(36).slice(2, 10);
    addAgent({
      id,
      name: newName,
      status: "active",
    ...
    <div className="space-y-6">
      ...
      <div className="relative">
        ...
        <Input
          ...
          className="pl-10 border-slate-700 bg-slate-800/50 text-white placeholder:text-slate-500"
        />
      </div>
                  ...
                  <tr key={agent.id} className="hover:bg-slate-800/30">
                        ...
                        <div className="h-2 w-16 rounded-full bg-slate-800">
                          <div
                            className={`h-full rounded-full ${
                              agent.trustScore >= 70 ? "bg-emerald-500" : agent.trustScore >= 40 ? "bg-amber-500" : "bg-red-500"
                            }`}
                    ...
                    <td className="p-4 text-right">
                      <div className="flex items-center justify-end gap-1">
                        <Button variant="ghost" size="icon" className="h-8 w-8 text-slate-400 hover:text-white" title="Rotate key">
        ...
        <DialogContent className="border-slate-800 bg-slate-900 text-white">
          ...
          <DialogFooter>
            ...
            <Button onClick={handleCreate} className="bg-blue-600 text-white hover:bg-blue-700">
              <Key className="mr-2 h-4 w-4" /> Create Agent
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={!!showRevoke} onOpenChange={() => setShowRevoke(null)}>
        <DialogContent className="border-slate-800 bg-slate-900 text-white">
          <DialogHeader>
            ...
            <DialogDescription className="text-slate-400">
              This will immediately invalidate all tokens and suspend all grants for this agent. This cannot be undone.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            ...
            <Button
              variant="destructive"
              onClick={() => {
                if (showRevoke) { revokeAgent(showRevoke); setShowRevoke(null); }
              }}
            >
              Revoke Agent
            </Button>
