        ...
        <AnimatePresence mode="wait">
          <motion.div key={step} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.2 }}>
            <Card className="border-hairline bg-surface/60"><CardContent className="p-6">
              {step === 0 && <div className="space-y-4"><div className="space-y-2"><Label htmlFor="wiz-agent-name">Agent Name</Label><Input id="wiz-agent-name" placeholder="e.g., Code Review Bot" value={agentName} onChange={(e) => setAgentName(e.target.value)} className="rounded-xl border-hairline bg-background" /></div><p className="text-xs text-muted-foreground">A key pair will be generated automatically.</p></div>}
              {step === 1 && <div className="space-y-4">
                <div className="space-y-2"><Label>Resource Type</Label><Select value={resourceType} onValueChange={setResourceType}><SelectTrigger aria-label="Resource type" className="rounded-xl border-hairline bg-background"><SelectValue /></SelectTrigger><SelectContent className="border-hairline bg-surface"><SelectItem value="database">Database</SelectItem><SelectItem value="repository">Repository</SelectItem><SelectItem value="email">Email</SelectItem><SelectItem value="api">API</SelectItem></SelectContent></Select></div>
                <div className="space-y-2"><Label htmlFor="wiz-resource-pattern">Resource Pattern</Label><Input id="wiz-resource-pattern" value={resourcePattern} onChange={(e) => setResourcePattern(e.target.value)} className="rounded-xl border-hairline bg-background" /></div>
                <div className="space-y-2"><Label>Allowed Actions</Label><div className="flex gap-2">{["read", "write", "delete", "execute"].map((a) => (<button key={a} type="button" aria-pressed={grantActions.includes(a)} onClick={() => toggleAction(a)} className={`rounded-full px-3 py-1.5 text-xs font-medium border transition-colors ${grantActions.includes(a) ? "border-foreground bg-foreground text-primary-foreground" : "border-hairline bg-background text-muted-foreground"}`}>{a}</button>))}</div></div>
              </div>}
